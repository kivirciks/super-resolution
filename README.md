...
<table border="1">
 ...
 Таким образом, EDSR оказалась самой легкой моделью по архитектуре, что подтверждается временем обучения. по параметру PNSR она незначительно уступает RDN, однако, в рамках лабораторной работы я буду делать упор именно на скорость работы алгоритма (одной десятой качества можно принебречь).
 Победитель: EDSR 🥳 
