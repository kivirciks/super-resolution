...
from tensorflow.keras.layers import concatenate, Input, Activation, Add, Conv2D, Lambda, Dense, BatchNormalization, LeakyReLU
from tensorflow.keras.models import Model
from tensorflow.keras.optimizers import Adam
...
class Trainer:
    # generator - Keras model, супермасштабирующая или генераторная сеть
    # discriminator - Keras model, дискриминатор, состязательная модель для функции потери
    # feature_extractor - Keras model, cеть экстракторов признаков для компонента глубоких признаков функции потери
    # lr_train_dir - путь к каталогу, в котором хранятся изображения LR для обученя
    # hr_train_dir - путь к каталогу, в котором хранятся изображения HR для обучения
    # lr_valid_dir - путь к каталогу, в котором хранятся изображения LR для тестирования
    # hr_valid_dir - путь к каталогу, в котором хранятся изображения HR для тестирования
    # learning_rate - float
    # loss_weights - словарь, веса для функции потерь. Содержит generator, discriminator и feature_extractor
    # logs_dir - директория, куда сохраняются логи
    # weights_dir - директория, куда сохраняются веса
    # dataname - string, используется для определения того, какой набор данных используется в этом обучения
    # weights_generator - веса для генератора
    # weights_discriminator - веса для дискриминатора
    # n_validation - integer, количество validation samples, использованных при обучении validation set
    # flatness - словарь, определяет пороговый уровень «равномерности» для тренировочных патчей (отсылка на TrainerHelper)
    # lr_decay_frequency: integer, через сколько эпох скорость обучения снижается
    # lr_decay_factor: 0 < float <1, мультипликативный коэффициент снижения скорости обучения
    ...
    # Создает комбинированную модель, которая содержит генераторную сеть, а также дискриминатор и экстрактор признаков, если таковые определены
   ...
   # Планировщик обновлений скорости обучения
    ...
    # Загружает предварительно обученные веса из заданного пути, если они есть. Если дискриминатор определен, делает то же самое
    ...
    # Создает словарь для отслеживания функции потерь
   ...
   # Подводит итоги обучения
    ...
    # Продолжает обучение в течение заданного количества эпох. Отправляет потери в Tensorboard.
    def train(self, epochs, steps_per_epoch, batch_size, monitored_metrics):
    # epochs - в течении скольких эпох идет обучение
    # steps_per_epoch - сколько батчев в каждой эпохе
    # batch_size - размер батча
    # monitored_metrics: словарь, отслеживание логики сохранения весов (минимум против максимума
        ...
        starting_epoch = self.helper.initialize_training(
            self)  
        # загружаем веса, создаем директории, создаем базовое имя
        ...
        for epoch in range(starting_epoch, epochs):
            ...
            # усреднение показателей обучения
            end_losses = {}
...
# Объект класса, который извлекает модель VGG19 keras, обученную на наборе данных imagenet, 
# Объявляет <layers_to_extract> в качестве выходных слоев. Использует функцию потерь
class Cut_VGG19:
# layers_to_extract - список слоев, которые должны быть объявлены как выходные слои
# patch_size - integer, определяет размер ввода (patch_size x patch_size)
    ...
    # загружает предварительно обученный VGG, объявляет в качестве вывода промежуточные слои, выбранные self.layers_to_extract
    def _cut_vgg(self):        
...
# Реализация дискриминатора для состязательной составляющей функции потери
class Discriminator:
# patch_size - integer, определяет размер ввода как (patch_size, patch_size, 3)
# kernel_size - размер kernel в conv блоках
# model - Keras model
# name - имя, используемое для определения того, какой дискриминатор используется во время обучения GAN
# block_param - словарь, определяет количество фильтров и шагов для каждого блока conv  
    ...
    # Объединяет слои дискриминатора
...
# ISR models parent class. Содержит функции, общие для моделей супермасштабирования.
class ImageModel:
    # Преобразует массив изображений в подходящий формат
    def predict(self, input_image_array, by_patch_of_size=None, batch_size=10, padding_size=2):
    # input_image_array - входящий массив изображений
    # by_patch_of_size - для вывода больших изображений. Разбивает изображение на фрагменты заданного размера
    # padding_size - для вывода больших изображений. Отступ между патчами. Нужно увеличить это значение, если есть накладка ("линия шва")
    # batch_size - для вывода больших изображений. Количество патчей, обрабатываемых за раз (лучше указывать маленькое значение, при необходимости увеличивая by_patch_of_size)
    # Returns: sr_img - выходное изображение     
        if by_patch_of_size:
...
# Объявление SRGAN
# https://arxiv.org/abs/1809.00219 (Wang et al. 2018)
class RRDN(ImageModel):
    # arch_params - словарь, содержит параметры нейронной сети C, D, G, G0, T, x
    # patch_size - integer or None, определяет размер ввода. Требуется только во время обучения, для прогнозирования установлено значение None
    # beta: float <= 1, параметр масштабирования для остаточных соединений
    # c_dim: integer, количество каналов входного изображения
    # kernel_size: integer, общий размер ядра для сверток
    # upscaling: string, 'ups' or 'shuffle', определяет, какую реализацию уровня масштабирования использовать
    # init_val: экстремальные значения для инициализатора RandomUniform
    #  weights: string, если не пусто, загрузка предварительно обученных весов
    ...
    # Реализация (остаточного) глубокого слоя, как в статье
    # d - integer, просто для названия (d-th block)
    ...
    # Реализация PixelShuffle части масштабирования
...
#rrdn.save_weights('srgan_weights.h5')

#import yadisk
#y = yadisk.YaDisk(token="y0_AgAAAAAZdSRIAAnWpQAAAADiIR-G69xDHp3vSUKGjYeHSNjcH6B_kQw")
# Сохранение весов
#rrdn.save_weights(y.upload('srgan_weights.h5', '/weights_dir/srgan_weights.h5'))
